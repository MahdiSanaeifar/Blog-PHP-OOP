...
class DataBase
{
    ...
    public function insertArticle($tableName, $fields, $values)
    {
        try {
            $stmt = $this->connection->prepare("INSERT INTO " . $tableName . "(" . implode(', ', $fields) . ") VALUES ( :" . implode(', :', $fields) . ");");
            $stmt->execute(array_combine($fields, $values));
            return true;
        } catch (PDOException $e) {
            echo "<div style='color:red;'> There is some problem in connection :</div>" . $e->getMessage();
